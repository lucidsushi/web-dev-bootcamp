...
-Define callback functions
    a function that is passed into another function as a parameter then
    invoked by that other function
...
-use a callback to make code more general
    some_function(param, callback) instead of bunch of functions(param)

-create callbacks using anonymous functions

l63
-describe and use foreach function
-implement the foreach function

l64
-describe and use `findIndex` function
    returns the index of the first element in the array for which the callback
    returns a truthy value, -1 is returned if the callback never returns a
    truthy value
-implement `findIndex`

l66
- describe what stack is
    - an ordered data structure
    - keeps track of function invocations
    - part of the javascript runtime (you dont access it directly)

    - when you invoke a function, the details of the invocation are saved to the
    top of the stack (pushed to the top)
    - whenever a function returns, the information about the invocation is
    taken off the top of the stack (popped off of the top)

    - an ordered set of stack frames, most recently invoked function is on the
    top of the stack, the bottom of the stack is the first function invoked,
    the stack is processed from top to bottom

- describe a stack frame
    - stack frame contents:
        (the stack instance for every invocation?)
...
- describe the heap
    ...
    # the object is created in the heap (taking memory),
    # obj is reference to the object
    var obj = {firstname: "Time"};

    # new data is not created, only a copy of the reference
    var referenceCopy = obj;

l67 (stack example)
    # methods being called in the code also count as invocations and therefore
    # adds to and pop off from stack

l68 setTimeout and setInterval
- write async code using both functions
    - setTimeout, a function that asynchronously invokes a callback after a
    delay in milliseconds

    # the second setTimeout cancels the first one!
    ...
    - setInterval, a function that continually invokes a callback after every
    x milliseconds, where x is provided to setInterval
